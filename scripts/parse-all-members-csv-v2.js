...
// ไฟล์ CSV ทั้ง 6 ไฟล์ (เวอร์ชันใหม่)
const csvFiles = [
  { 
    path: 'C:\\Users\\funny\\Downloads\\รายชื่อสมาชิก ปากเกร็ด tel(Sheet1) (1).csv', 
    startLine: 3,
    type: 'ปากเกร็ด'
  },
  { 
    path: 'C:\\Users\\funny\\Downloads\\รายชื่อสมาชิก บางกรวย tel 2(Sheet1) (1).csv', 
    startLine: 3,
    type: 'บางกรวย'
  },
  { 
    path: 'C:\\Users\\funny\\Downloads\\รายชื่อสมาชิก นนทบุรี  บางใหญ๋  tel(Sheet1) (1).csv', 
    startLine: 2,
    type: 'บางใหญ่'
  },
  { 
    path: 'C:\\Users\\funny\\Downloads\\รายชื่อสมาชืก บางบัวทอง tel(1)(Sheet1).csv', 
    startLine: 3,
    type: 'บางบัวทอง'
  },
  { 
    path: 'C:\\Users\\funny\\Downloads\\รายชื่อสมาชิก อเมือง  tel (Sheet1) (1).csv', 
    startLine: 3,
    type: 'อเมือง'
  },
  { 
    path: 'C:\\Users\\funny\\Downloads\\รายชื่อสมาชิก อ ไทรน้อย  tel(Sheet1) (1).csv', 
    startLine: 3,
    type: 'ไทรน้อย'
  },
];
...
for (const fileInfo of csvFiles) {
  ...
  for (let i = fileInfo.startLine - 1; i < lines.length; i++) {
    ...
    let memberId, firstName, lastName, memberType, phone;
    let houseNumber = '', village = '', villageName = '', soi = '', road = '', subdistrict = '';
    const district = fileInfo.type; // ใช้ชื่ออำเภอจาก fileInfo.type
    
    if (fileInfo.type === 'ปากเกร็ด') {
      // ปากเกร็ด: index 1=memberId, 6=ชื่อ, 7=นามสกุล, 8=ประเภท, 9=เบอร์, 10=ตำบล, 11=หมู่บ้าน, 12=ซอย, 13=ถนน
      // ไม่มีบ้านเลขที่และหมู่แยก
      ...
      memberType = parts[8] || '';
      phone = parts[9] || '';
      subdistrict = parts[10] || ''; // ตำบล
      houseNumber = ''; // ไม่มีใน CSV นี้
      village = ''; // ไม่มีใน CSV นี้
      villageName = parts[11] || '';
      soi = parts[12] || '';
      road = parts[13] || '';
      ...
      soi = parts[14] || '';
      road = parts[15] || '';
    } else if (fileInfo.type === 'บางบัวทอง') {
      // บางบัวทอง: index 1=memberId, 6=ชื่อ, 7=นามสกุล, 8=ประเภท, 9=เบอร์, 10=บ้านเลขที่, 11=หมู่, 12=หมู่บ้าน, 13=ซอย, 14=ถนน, 15=ตำบล
      if (parts.length < 11) continue;
      memberId = parts[1] || '';
      firstName = parts[6] || '';
      lastName = parts[7] || '';
      memberType = parts[8] || '';
      phone = parts[9] || '';
      houseNumber = parts[10] || '';
      village = parts[11] || '';
      villageName = parts[12] || '';
      soi = parts[13] || '';
      road = parts[14] || '';
      subdistrict = parts[15] || ''; // ตำบล
    ...
    if (memberId && firstName && lastName && memberType) {
...
console.log(`   - district = อำเภอ, subdistrict = ตำบล`);
