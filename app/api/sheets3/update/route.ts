...
// Sheet ID และ GID จาก URL สำหรับหน้า incident report
const SHEET_ID = '104j-UWwg5mwAUwCwAWAVr-Hw6Ggbj9I9KpqRUra9ITA';
const GID = '1813370749';
...
export async function POST(request: Request) {
  try {
    ...
    console.log('Environment check (sheets3):');
    ...
    // Note: คุณอาจต้องสร้าง Apps Script URL ใหม่สำหรับ Sheet นี้ หรือใช้ URL เดียวกัน
    ...
    try {
      ...
      const response = await fetch(finalAppsScriptUrl, {
        ...
        body: JSON.stringify({
          ...
          value: value,
          sheetId: SHEET_ID, // ส่ง Sheet ID เพื่อให้ Apps Script รู้ว่าใช้ Sheet ไหน
          gid: GID,
        }),
      });
