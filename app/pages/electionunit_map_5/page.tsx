...
const ElectionUnitMap5 = dynamic(
  () => import("../../components/ElectionUnitMap5"),
  { ssr: false }
);

export default function ElectionUnitPage_MAP_5() {
  ...
  // Filter to only district 5 units
  const district5Units = useMemo(() => {
    return electionUnits.filter(u => u.electionDistrict === "5");
  }, [electionUnits]);

  // Get unique values for dropdowns
  const districts = useMemo(() => {
    // Get unique districts and filter out header names and invalid values
    const unique = Array.from(new Set(district5Units.map(u => u.district).filter(Boolean)));
  ...
  }, [district5Units, selectedAmphoe, selectedSubdistrict]);
  ...
  const filteredUnits = useMemo(() => {
    ...
    return filtered;
  }, [district5Units, selectedAmphoe, selectedSubdistrict, selectedVillage, searchQuery]);
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-3xl sm:text-4xl font-bold text-orange-900 mb-2">
              ข้อมูลหน่วยเลือกตั้ง เขต 5
            </h1>
            <p className="text-orange-700 text-sm sm:text-base">
              แผนที่และรายละเอียดหน่วยเลือกตั้งเขต 5
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg border-2 border-orange-200 mb-6">
            ...
            <ElectionUnitMap5 units={filteredUnits} />
          </div>
